...
void GoleiroBehavior::VerQueda()
{
    ...
    else
    {
        ...
        else
        {
            ...
            RCLCPP_INFO(this->get_logger(), "VerQueda: caso 2 -> zona intermediária (pos19=%d) -> squat", robot.neck_pos.position19);
            send_goal(g_squat); // "Goalkeeper Middle" (control.cpp case 13) — bloqueio central
            queda_get_up_move_ = g_stand_up_front; // bloqueio central levanta como uma queda de frente
        }
    }
