...
DecisionNode::DecisionNode() : Node("decision_node")
{
 ...
 imu_mag_subscriber_ = this->create_subscription<geometry_msgs::msg::Vector3Stamped>(
 "/imu/mag", rclcpp::QoS(10),
 std::bind(&DecisionNode::listener_callback_imu_mag, this, std::placeholders::_1)); 
...
void DecisionNode::listener_callback_imu_mag(const geometry_msgs::msg::Vector3Stamped::SharedPtr mag)
{
 robot.imu_mag = mag->vector.x;
 RCLCPP_INFO(this->get_logger(), "🧭 Yaw recebido: %.3f rad", robot.imu_yaw_rad);
}
